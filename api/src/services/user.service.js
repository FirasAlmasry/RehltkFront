...
export const isNewUser = async (email) => {
    const user = await User.findOne({ email });
    if (user) throw new ErrorHandler(400, "user already exist!");
};

export const isUserRegisterDataValide = async (data) => {
    const { error, value } = userRegisterValidation(data);
    if (error) throw new ErrorHandler(400, error.details[0].message);
};
export const isLogInDataValide = async (data) => {
    const { error, value } = userLogInValidation(data);
    if (error) throw new ErrorHandler(400, error.details[0].message);
};
